...
public class InventoryPopup : MonoBehaviour {
	[SerializeField] private Image[] itemIcons = null;
	[SerializeField] private Text[] itemLabels = null;
	...
	public void Refresh() {
		...
		for (int i = 0; i < len; i++) {
			if (i < itemList.Count) {
				...
				if (item == Managers4.Inventory.equippedItem) {
					message = "Equipped\n" + message;
				}
	...
	public void OnEquip() {
		Managers4.Inventory.EquipItem(_curItem);
		Refresh();
	}

	public void OnUse() {
		Managers4.Inventory.ConsumeItem(_curItem);
		if (_curItem == "health") {
